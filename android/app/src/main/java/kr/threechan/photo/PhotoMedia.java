...
/** Only image IDs obtained from MediaStore are readable. No arbitrary URI/path bridge. */
public final class PhotoMedia {
    ...
    private boolean enabled(){return BackgroundSyncWorker.enabled(context);}
    @JavascriptInterface public synchronized String list(){
        ...
        String[] columns={MediaStore.Images.Media._ID,MediaStore.Images.Media.DISPLAY_NAME,MediaStore.Images.Media.SIZE,MediaStore.Images.Media.DATE_MODIFIED};
        try(SQLiteDatabase db=database();Cursor cursor=context.getContentResolver().query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI,columns,null,null,MediaStore.Images.Media.DATE_ADDED+" ASC")){
            ...
            while(cursor.moveToNext()&&result.length()<12){
                ...
                if(size<=0||size>250L*1024*1024)continue;
                String filename=cursor.getString(1);if(filename==null||!filename.toLowerCase(java.util.Locale.ROOT).matches(".*\\.(jpe?g|png|webp|heic|heif|avif|gif|tiff?)$"))continue;
                ...
                if(Build.VERSION.SDK_INT>=29&&context.checkSelfPermission("android.permission.ACCESS_MEDIA_LOCATION")==android.content.pm.PackageManager.PERMISSION_GRANTED)uri=MediaStore.setRequireOriginal(uri);
    ...
    @JavascriptInterface public synchronized String read(String id,long offset,int length){
        if(!enabled()||!permitted.containsKey(id)||offset<0||length<1||length>1024*1024)throw new SecurityException("사진 전송 범위가 올바르지 않아요.");
    ...
    @JavascriptInterface public synchronized void complete(String id){
        if(!enabled()||!versions.containsKey(id))throw new SecurityException();

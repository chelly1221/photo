...
원본 삭제 기능 배포 시 `scripts/photo-nas.py`도 기존 도우미 경로 `/usr/local/sbin/photo-nas`에 root 소유·0755 권한으로 갱신해야 합니다. 배포 전 기존 도우미를 보관하고 롤백할 때 API와 함께 되돌립니다. 새 API만 배포하면 `delete-file` 작업이 실패합니다.

사진·동영상 길게 누르기에서 사용자가 원본 삭제를 확인하면 도우미가 저장된 계정과 공유 폴더 범위 안의 파일 하나만 삭제합니다. 감상용 마운트의 읽기 전용 설정은 유지합니다. 파일 크기·수정 시각과 연결 소유자를 검사하며 폴더 삭제는 지원하지 않습니다. 성공 후 인덱스·앨범 참조·파생 파일을 제거합니다. NAS 계정에 삭제 권한이 있어야 하며, 이전 수동 CIFS 연결은 앱에서 NAS 공유 폴더를 다시 연결해야 합니다. 앱 내 복구 기능은 없습니다.

동영상 썸네일은 앞부분의 대표 프레임을 추출하고 실패하면 첫 프레임을 다시 시도합니다. 2GiB를 초과한 동영상도 썸네일·미리보기는 생성하되 앱 재생용 MP4 복사본은 만들지 않습니다. 기존 코덱 실패 항목은 백그라운드 재시도 스캔에서 처리됩니다.
...
- 웹/Android 로그아웃은 Tailscale 인증을 종료하고 자동 백업을 중지합니다. 기기의 사진 목록·썸네일 캐시와 백업 대상 폴더·Wi-Fi 설정은 보존합니다. NAS 원본도 지우지 않습니다.
